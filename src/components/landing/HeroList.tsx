...
const HoverList = () => {
    ...
    return (
        <nav className="h-screen flex justify-center items-center font-subtitle">
                ...
                {items.map((item, index) => (
                    <li key={index} className="relative text-[42px] leading-[1.5] group">
                        ...
                        <span className="absolute top-1/2 transform -translate-y-1/2 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <FaArrowRight className="w-6 h-6 text-primary transition-transform duration-500 group-hover:translate-x-2" />
                        </span>
                    </li>
                ))}
            </ul>
